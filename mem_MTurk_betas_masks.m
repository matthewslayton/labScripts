...
% want to check diff measures of mem and the single-trial betas and prepare
% them to be loaded into SPM for further analysis

% see factors_betas_forSPM.m which is similar code for preparing factors
% for SPM
...
visMem_HR_subset_S002 = zeros(length(indices_inOrder),1); 
visMem_FAR_subset_S002 = zeros(length(indices_inOrder),1); 
lexMem_HR_subset_S002 = zeros(length(indices_inOrder),1); 
lexMem_FAR_subset_S002 = zeros(length(indices_inOrder),1); 
visMem_CR_subset_S002 = zeros(length(indices_inOrder),1); 
lexMem_CR_subset_S002 = zeros(length(indices_inOrder),1);
PCs_S002 = zeros(length(indices_inOrder),994);

for row = 1:length(indices_inOrder)
    visMem_HR_subset_S002(row,:) = visMem_HR(indices_inOrder(row),:);
    visMem_FAR_subset_S002(row,:) = visMem_FAR(indices_inOrder(row),:);
    lexMem_HR_subset_S002(row,:) = lexMem_HR(indices_inOrder(row),:);
    lexMem_FAR_subset_S002(row,:) = lexMem_FAR(indices_inOrder(row),:);
    visMem_CR_subset_S002(row,:) = visMem_CR(indices_inOrder(row),:);    
    lexMem_CR_subset_S002(row,:) = lexMem_CR(indices_inOrder(row),:);
    PCs_S002(row,:) = score(indices_inOrder(row),:);

end

save('visMem_HR_subset_S002','visMem_HR_subset_S002')
save('visMem_FAR_subset_S002','visMem_FAR_subset_S002')
save('visMem_CR_subset_S002','visMem_CR_subset_S002')
save('lexMem_HR_subset_S002','lexMem_HR_subset_S002')
save('lexMem_FAR_subset_S002','lexMem_FAR_subset_S002')
save('lexMem_CR_subset_S002','lexMem_CR_subset_S002')
save('PCs_S002','PCs_S002')
...
visMem_HR_S002_SPM = visMem_HR_subset_S002(~isnan(lexMem_HR_subset_S002));
...
% CR
lexMem_CR_S002_SPM = lexMem_CR_subset_S002(~isnan(lexMem_CR_subset_S002));
...
visMem_CR_S002_SPM = visMem_CR_subset_S002(~isnan(lexMem_CR_subset_S002));
...
% visMem_CR_S0XX_SPM NaN indices
% S002_SPM: 27, 53, 223
% S005_SPM: 101, 138, 214
% S013_SPM: 24, 111, 215
% S023_SPM: 3, 31, 227
...
lexMem_CR_S002_SPM_clean = lexMem_CR_S002_SPM(~isnan(visMem_CR_S002_SPM));
...
visMem_CR_S002_SPM_clean = visMem_CR_S002_SPM(~isnan(visMem_CR_S002_SPM));
...
save lexMem_CR_S002_SPM
save lexMem_CR_S005_SPM
save lexMem_CR_S013_SPM
save lexMem_CR_S023_SPM
save visMem_CR_S002_SPM
save visMem_CR_S005_SPM
save visMem_CR_S013_SPM
save visMem_CR_S023_SPM

save lexMem_CR_S002_SPM_clean
save lexMem_CR_S005_SPM_clean
save lexMem_CR_S013_SPM_clean
save lexMem_CR_S023_SPM_clean
save visMem_CR_S002_SPM_clean
save visMem_CR_S005_SPM_clean
save visMem_CR_S013_SPM_clean
save visMem_CR_S023_SPM_clean

%Has 244
%/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/Encoding_renamed/S005/betas_smoothed_noLexCRNaN

%Has 241
%/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/Encoding_renamed/S005/betas_smoothed_both

%% use these mem values as covariates when doing mult reg in SPM
addpath '/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/visLexMem_groups'
load lexMem_CR_S002_SPM
load lexMem_CR_S005_SPM
load lexMem_CR_S013_SPM
load lexMem_CR_S023_SPM
load visMem_CR_S002_SPM
...
% HR
meanCtr_vis_HR_S002 = visMem_HR_S002_SPM - mean(visMem_HR_S002_SPM);
...
meanCtr_lex_HR_S002 = lexMem_HR_S002_SPM - mean(lexMem_HR_S002_SPM);
...
meanCtr_both_HR_S002 = meanCtr_vis_HR_S002 .* meanCtr_lex_HR_S002;
...
save('meanCtr_both_HR_S002','meanCtr_both_HR_S002')
...
% CR
meanCtr_vis_CR_S002 = visMem_CR_S002_SPM_clean - mean(visMem_CR_S002_SPM_clean);
...
meanCtr_lex_CR_S002 = lexMem_CR_S002_SPM_clean - mean(lexMem_CR_S002_SPM_clean);
...
meanCtr_both_CR_S002 = meanCtr_vis_CR_S002 .* meanCtr_lex_CR_S002;
...
save('meanCtr_both_CR_S002','meanCtr_both_CR_S002')
...
load meanCtr_both_CR_S002
load meanCtr_both_CR_S005
load meanCtr_both_CR_S013
load meanCtr_both_CR_S023

meanCtr_both_CR = (meanCtr_both_CR_S002+meanCtr_both_CR_S005+meanCtr_both_CR_S013+meanCtr_both_CR_S023)./4;
save('meanCtr_both_CR.mat','meanCtr_both_CR')


%% sort .nii files from betas_smoothed into folders. Cut down from 300 or cut from 244 to 241

% cut down from 300
subjectNum_002 = {'002'};
subjects = 1;
counter = 0;
data_dir_enc = strcat('/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/Encoding_renamed/S',subjectNum_002{subjects},'/betas_smoothed/'); % just folder, not content yet. 
all_betas_enc = dir(strcat('/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/Encoding_renamed/S',subjectNum_002{subjects},'/betas_smoothed/*.nii')); % dir gives the file names

for i = 1:300
   
    cd(strcat('/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/Encoding_renamed/S',subjectNum_002{subjects},'/betas_smoothed/'));

    source = all_betas_enc(i).name;
    % don't forget to make these folders in the proper place so they can be filled
    destination_HR = strcat('/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/Encoding_renamed/S',subjectNum_002{subjects},'/betas_smoothed_noLexNaN/');
    destination_CR = strcat('/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/Encoding_renamed/S',subjectNum_002{subjects},'/betas_smoothed_noLexCRNaN/');
   
    if isnan(lexMem_HR_subset_S002(i)) % is this value equal to NaN?
        counter = counter + 1;
    else
        copyfile(source,destination_HR); 
    end
    if isnan(lexMem_CR_subset_S002(i))
        counter = counter + 1;
    else
...
% now reduce the 244 to 241 because visMem_CR_S0XX_SPM has three NaNs. The
% four stimulus groups have them in different places.
subjectNum_002 = {'002'};
subjects = 1;
counter = 0;
data_dir_enc = strcat('/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/Encoding_renamed/S',subjectNum_002{subjects},'/betas_smoothed/'); % just folder, not content yet. 
...
for i = 1:244
    ...
    source = all_betas_enc(i).name;
    destination_both = strcat('/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/Encoding_renamed/S',subjectNum_002{subjects},'/betas_smoothed_both/');

    if isnan(visMem_CR_S002_SPM(i))
        counter = counter + 1;
    else
        copyfile(source,destination_both);
    end
end
...
for subjects = 1:length(subjectNum_005)
    ...
    for i = 1:244
        ...
        source = all_betas_enc(i).name;
        destination_both = strcat('/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/Encoding_renamed/S',subjectNum_005{subjects},'/betas_smoothed_both/');
    
        if isnan(visMem_CR_S005_SPM(i))
            counter = counter + 1;
        else
            copyfile(source,destination_both);
        end
...
% cut from 244 to 241
subjectNum_013 = {'014' '015' '016' '018' '019' '021' '022'}; %'013'
for subjects = 1:length(subjectNum_013)
    counter = 0;
    data_dir_enc = strcat('/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/Encoding_renamed/S',subjectNum_013{subjects},'/betas_smoothed/'); % just folder, not content yet. 
    all_betas_enc = dir(strcat('/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/Encoding_renamed/S',subjectNum_013{subjects},'/betas_smoothed/*.nii')); % dir gives the file names
    ...
    for i = 1:244
        ...
        source = all_betas_enc(i).name;
        destination_both = strcat('/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/Encoding_renamed/S',subjectNum_013{subjects},'/betas_smoothed_both/');
    
        if isnan(visMem_CR_S013_SPM(i))
            counter = counter + 1;
        else
            copyfile(source,destination_both);
        end
...
for subjects = 1:length(subjectNum_023)
    ...
    for i = 1:244
        ...
        source = all_betas_enc(i).name;
        destination_both = strcat('/Users/matthewslayton/Library/CloudStorage/OneDrive-DukeUniversity/STAMP/Encoding_renamed/S',subjectNum_023{subjects},'/betas_smoothed_both/');
    
        if isnan(visMem_CR_S023_SPM(i))
            counter = counter + 1;
        else
            copyfile(source,destination_both);
        end
...
% also use as general example for making masks
...
% insR
insR_1 = load_untouch_nii('INS_R_6_1.nii');
insR_2 = load_untouch_nii('INS_R_6_2.nii');
insR_3 = load_untouch_nii('INS_R_6_3.nii');
insR_4 = load_untouch_nii('INS_R_6_4.nii');
insR_5 = load_untouch_nii('INS_R_6_5.nii');
insR_6 = load_untouch_nii('INS_R_6_6.nii');
% don't forget to add the header info
insR.('hdr') = insR_1.hdr;
insR.('filetype') = 2;
insR.('fileprefix') = 'subcortical';
insR.('machine') = 'ieee-le';
insR.('ext') = [];
insR.img = insR_1.img+insR_2.img+insR_3.img+insR_4.img+insR_5.img+insR_6.img;
insR.('untouch') = 1;
% save
save_untouch_nii(insR,'insR.nii')
...
% pcunR
PcunR_1 = load_untouch_nii('Pcun_R_4_1.nii');
PcunR_2 = load_untouch_nii('Pcun_R_4_2.nii');
PcunR_3 = load_untouch_nii('Pcun_R_4_3.nii');
PcunR_4 = load_untouch_nii('Pcun_R_4_4.nii');
% don't forget to add the header info
PcunR.('hdr') = PcunR_1.hdr;
PcunR.('filetype') = 2;
PcunR.('fileprefix') = 'subcortical';
PcunR.('machine') = 'ieee-le';
PcunR.('ext') = [];
PcunR.img = PcunR_1.img+PcunR_2.img+PcunR_3.img+PcunR_4.img;
PcunR.('untouch') = 1;
% save
save_untouch_nii(PcunR,'PcunR.nii')
