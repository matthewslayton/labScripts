...
%% note: this code uses the STAMP memorability data.
set(0,'defaultfigurecolor',[1 1 1]) %set background of plot to white


%% STAMP memorability
% lower down is the MTurk normative study memorability dta
% both can use the same PC code
...
for subjects = 1:length(subjectNum) 
    ...
    items_and_cmemMem = [IDs,memorability];
    
    % sort so they're in alphabetical order
    items_and_cmem_sorted = sortrows(items_and_cmemMem,1);
    
    %save(strcat('items_cmem_S',subjectNum{subjects}),'items_and_cmem_sorted')
end
...
full_memSubj = horzcat(itemMem_val_002(:,2),itemMem_val_005(:,2),itemMem_val_006(:,2),itemMem_val_008(:,2),itemMem_val_009(:,2),itemMem_val_010(:,2),itemMem_val_011(:,2),itemMem_val_013(:,2),itemMem_val_014(:,2),itemMem_val_015(:,2),itemMem_val_016(:,2),itemMem_val_018(:,2),itemMem_val_019(:,2),itemMem_val_021(:,2),itemMem_val_022(:,2),itemMem_val_023(:,2)); %index second col, the memorability data
    
memMean = mean(full_memSubj,2,'omitnan');
% this is 300x1. Has a memorability score for all 300 items
% I made the missing and catch trials NaN, so it was easy to skip them
% over, especially in calculating the mean. It doesn't include the NaN
% subject in the total subject count for that row

% remove the NaNs that remain because the NaNs on the memorability matrix
% are catch trials. Every subject had NaN for that item, so it must not
...
memMean_330_withIDs_cmem = [itemMem_val_021(:,1),memMean];
save('mem330_withIDs_cmem','memMean_330_withIDs_cmem')

memMean_360_withIDs_cmem = [itemMem_val_021(:,1),memMean];
save('mem360_withIDs_cmem','memMean_360_withIDs_cmem')

% skip this next section if you want. You can just load the val it
% calculates
%% PCs for each item 
% will be rows from the PCA
% I make the PCs in mds_practice.m or make_PCs.m
...
%%% wherever there is a NaN in memMean, I have to remove that row from
%%% score_subset
locations = isnan(memMean);
score_subset_NaN = zeros(330,length(score_subset));
for number = 1:330
    if locations(number) == 0
        score_subset_NaN(number,:) = score_subset(number,:);
    else
        score_subset_NaN(number,:) = NaN;
    end

end

% remove the rows with NaN
score_subset_NaN(any(isnan(score_subset_NaN),2),:)= [];

score_subset_300 = score_subset_NaN;


%% Need item-wise PC score subsets to match the STAMP subject items
...
load('score_subset_IDsInOrder.mat')
...
mdl = fitlm(score_subset_300 (:,1:num_PCs),memMean_300);
