...
for subjects = 1:length(subjectNum) 
    ...
    memorability = zeros(length(pmem_ret_col),1);
    for number = 1:length(pmem_ret_col)
        if old(number) == 1 %old refers to the fact that they've seen these before during enc
            if pmem_ret_col(number) == 1 %0 for miss
                memorability(number) = 0;
            elseif pmem_ret_col(number) == 2 
                memorability(number) = 0;
            elseif pmem_ret_col(number) == 3 %1 for hit
                memorability(number) = 1;
            elseif pmem_ret_col(number) == 4  %3 for hit
                memorability(number) = 1;
    ...
    items_and_pmemMem = [IDs_old,mem_old];
    
    items_and_pmem_sorted = sortrows(items_and_pmemMem,1);
    
    save(strcat('items_pmem_ret_S',subjectNum{subjects}),'items_and_pmem_sorted')
    %save(strcat('IDs_old_S',subjectNum{subjects}),'IDs_old')
    %save(strcat('IDs_S',subjectNum{subjects},'withNaN'),'IDs')
    
end

%% take average across subjects for each item
itemMem_002 = load('items_pmem_S002.mat'); %loads as a struct
itemMem_val_002 = itemMem_002.items_and_pmem_sorted; %have to get the data out of the struct
...
memMean_pmem = mean(full_memSubj_pmem,2);




save('memorability_scores_pmem','memMean_pmem')
...
indices_inOrder = zeros(length(memMean_pmem),1);
for idNumber = 1:length(memMean_pmem)
   ...
   % indices_inOrder gives you the indices of the IDs that match those
    % from enc
...
mdl = fitlm(score_subset(:,1:num_PCs),memMean_pmem);
