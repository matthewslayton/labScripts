function NetTMS_RETtask1_perceptual
% To Do
...
ITIfixationDuration = jitterITI(numtrials); %before38 % Lengths of fixation in seconds before every trial, with jitter
...
pdata.run_no=input('Enter run number (1-3): ');
subid = pdata.sub_no;
assert(isnumeric(pdata.run_no))

resultsFolder = num2str(subid);
if exist(fullfile(resultsFolder,[num2str(subid) '_day' num2str(pdata.day_no) '_run' num2str(pdata.run_no) 'PRET_output.mat']),'file')
...
inputdata = sprintf('%d/%d_day%d_stimtbl_PRET.csv', subid, subid, pdata.day_no);
...
imgI = imread(fullfile('instructionPics','RET_Pinstructions.png'));
...
DrawFormattedText(window1, 'Press any spacebar when ready','center', 900, textColor)
Screen('Flip',window1)
...
%     imageSizeO = imageSizeO*1.5;
    ...
    if keyCode(ButEsc)
            ...
            save(fullfile(resultsFolder,[pdata.s_ID_n_Run '_PRET_output_esc.mat']), 'output_tbl')
            sca;
            return
    end
    ...
    output.rt{t,1}=rt;
    output.tObj1Onset{t,1}=tObject1Onset;% this is the onset of object image
...
% WaitSecs(4); % at the end of the trials add some time of just blank screen
output_tbl = struct2table(output); 
save(fullfile(resultsFolder,[pdata.s_ID_n_Run '_PRET_output.mat']), 'output_tbl')
